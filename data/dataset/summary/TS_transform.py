...
# 删除非中文字符，以及标点符号
def clean_Non_CHINESE_PUN(line):
    return ''.join(re.findall("([\u4e00-\u9fa5]+)+?", line))


def T2S(line):  # 将繁体转换成简体
    return Converter('zh-hans').convert(line.replace('\n', ''))


def T2S_NCP(line):  # 将繁体转换成简体
    return clean_Non_CHINESE_PUN(Converter('zh-hans').convert(line.replace('\n', '')))


def S2T(line):  # 将简体转换成繁体
    return Converter('zh-hant').convert(line.replace('\n', ''))


def S2T_NCP(line):  # 将简体转换成繁体
    return clean_Non_CHINESE_PUN(Converter('zh-hant').convert(line.replace('\n', '')))
